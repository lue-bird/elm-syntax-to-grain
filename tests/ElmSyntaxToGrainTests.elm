module ElmSyntaxToGrainTests exposing (suite)
...
suite : Test
suite =
    Test.describe "elm-syntax-to-grain"
...
a2 x =
    case x of
        ...
        _ ->
            1
"""
                    |> expectTranspiledToGrainStringAs
                        """namespace global
...
expectTranspiledToGrainStringAs : String -> String -> Expect.Expectation
expectTranspiledToGrainStringAs expected source =
    case
                    ...
                    , declarations :
                        { valuesAndFunctions :
                            FastDict.Dict
                                ...
                                , type_ : Maybe ElmSyntaxToGrain.GrainType
                                }
                        , typeAliases :
                            FastDict.Dict
                                String
                        ...
                        , enumTypes :
                            FastDict.Dict
                                String
                                { parameters : List String
                                , variants : FastDict.Dict String (List ElmSyntaxToGrain.GrainType)
                                }
                        }
                    }
                transpiledResult =
                    parsedModules |> ElmSyntaxToGrain.modules
            in
            case transpiledResult.errors of
                transpilationError0 :: transpilationError1Up ->
                    Expect.fail
                        ("failed to transpile the parsed elm to grain: "
                            ++ ((transpilationError0 :: transpilationError1Up)
                                    |> String.join " and "
                               )
                        )
